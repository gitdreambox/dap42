...
static const char** webusb_http_urls;
static size_t webusb_num_http_urls;
...
static int webusb_control_vendor_request(usbd_device *usbd_dev,
    ...
    switch (req->wIndex) {
        case WEBUSB_REQ_GET_ALLOWED_ORIGINS: {
            static struct webusb_simple_origins origins = {
                ...
                .function_subset_header = {
                    ...
                    .bFirstInterfaceNumber = INTF_HID,
                },
                .iOrigin = { 1 }
            };
        ...
        case WEBUSB_REQ_GET_URL: {
            ...
            if (index < webusb_num_http_urls) {
                size_t url_len = strlen(webusb_http_urls[index]);
                url->bLength = WEBUSB_DT_URL_DESCRIPTOR_SIZE + url_len;
                url->bDescriptorType = WEBUSB_DT_URL;
                url->bScheme = WEBUSB_URL_SCHEME_HTTP;
                memcpy(&url->URL, webusb_http_urls[index], url_len);
                *len = url->bLength;
                status = USBD_REQ_HANDLED;
            } else if (index < webusb_num_http_urls + webusb_num_https_urls) {
                index -= webusb_num_http_urls;
...
void webusb_setup(const char** http_urls, size_t num_http_urls,
                  const char** https_urls, size_t num_https_urls
    ) {
    webusb_http_urls = http_urls;
    webusb_https_urls = https_urls;

    webusb_num_http_urls = (http_urls == NULL) ? 0 : num_http_urls;
    webusb_num_https_urls = (https_urls == NULL) ? 0 : num_https_urls;

    cmp_usb_register_set_config_callback(webusb_set_config);
}
